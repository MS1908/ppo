...
for i in range(t_range):
    ...
    rewards_energy_list_ppo.append(e)
    avg_rewards_energy_list_ppo.append(np.mean(rewards_energy_list_ppo[:]))
    if dones: env.reset()
    # env.render()
print('--RESULTS--')
...
df=pd.DataFrame({'x': range(t_range), 'y_1': avg_rewards_ppo,'y_2': avg_rewards_time_list_ppo, 'y_3': avg_rewards_energy_list_ppo})
plt.xlabel("Time Slot")
plt.ylabel("Time Average Cost")
plt.plot( 'x', 'y_1', data=df, marker='o', markevery = 700, color='red', linewidth=1, label="ppo")
plt.plot( 'x', 'y_2', data=df, marker='^', markevery = 700, color='cyan', linewidth=1, label="time")
plt.plot( 'x', 'y_3', data=df, marker='+', markevery = 700, color='g', linewidth=1, label="energy")
