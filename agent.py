...
for i in range(t_range):
    ...
    avg_rewards_fixed_2.append(np.mean(rewards_list_fixed_2[-100:-1]))
    if dones: env.reset()
...
for i in range(t_range):
	...
	avg_rewards_random.append(np.mean(rewards_list_random[-100:-1]))
	if dones: env.reset()
...
for i in range(t_range):
    ...
    avg_rewards_ppo.append(np.mean(rewards_list_ppo[-100:-1]))
    if dones: env.reset()
    # env.render()
