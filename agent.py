...
model.learn(total_timesteps=96000)
...
for i in range(t_range):
    ...
    avg_rewards_fixed_2.append(np.mean(rewards_list_fixed_2[-1000:-1]))
    if dones: env.reset()
...
for i in range(t_range):
	...
	avg_rewards_random.append(np.mean(rewards_list_random[-1000:-1]))
	if dones: env.reset()
...
for i in range(t_range):
    ...
    avg_rewards_ppo.append(np.mean(rewards_list_ppo[-1000:-1]))
    if dones: env.reset()
    # env.render()
