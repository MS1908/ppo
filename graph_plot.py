...
try:
    ...
    i = 0
    rng = 0
    with open(os.path.join(my_path, xls_file), 'rb') as f:
        xls_data = pd.read_excel(f)
        for i in range(6):
            plot_list[i] = xls_data['y_' + str(i + 1)].tolist()
            rng = len(plot_list[i])
        i += 1
    df = pd.DataFrame(
    ...
    plt.savefig(os.path.join(my_path, fig_file))
    plt.show()
except IOError:
    print('Fuck')
...
for name in xls_file_names:
    try:
        with open(os.path.join(my_path, name), 'rb') as f:
            xls_data = pd.read_excel(f)
            plot_list[i] = xls_data['y_1'].tolist()
            rng = len(plot_list[i])
            i += 1
    except IOError:
        print('Fuck')
...
xx = range(rng)
fig = plt.stackplot(xx, plot_list, colors = 'w', edgecolor = 'black', labels = ['Delay cost', 'Backup cost', 'Battery cost'])
hatches = ['...', '+++++', '///']
...
for name in xls_file_names:
    try:
        with open(os.path.join(my_path, name), 'rb') as f:
            xls_data = pd.read_excel(f)
            plot_list[i] = xls_data['y_2'].tolist()
            rng = len(plot_list[i])
            i += 1
    except IOError:
        print('Fuck')
...
for name in xls_file_names:
    try:
        with open(os.path.join(my_path, name), 'rb') as f:
            xls_data = pd.read_excel(f)
            plot_list[i] = xls_data['y_3'].tolist()
            rng = len(plot_list[i])
            i += 1
    except IOError:
        print('Fuck')
...
for name in xls_file_names:
    try:
        with open(os.path.join(my_path, name), 'rb') as f:
            xls_data = pd.read_excel(f)
            plot_list[i] = xls_data['y_4'].tolist()
            rng = len(plot_list[i])
            i += 1
    except IOError:
        print('Fuck')
...
for name in xls_file_names:
    try:
        with open(os.path.join(my_path, name), 'rb') as f:
            xls_data = pd.read_excel(f)
            plot_list[i] = xls_data['y_5'].tolist()
            rng = len(plot_list[i])
            i += 1
    except IOError:
        print('Fuck')
...
for name in xls_file_names:
    try:
        with open(os.path.join(my_path, name), 'rb') as f:
            xls_data = pd.read_excel(f)
            plot_list[i] = xls_data['y_6'].tolist()
            rng = len(plot_list[i])
            i += 1
    except IOError:
        print('Fuck')
